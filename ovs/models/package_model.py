...
class Package(app.BaseModel):
    ...
    def __repr__(self):
        return 'Package([id="%s", recipient_id="%s", checked_by_id="%s", checked_at="%s", is_signed="%s", signed_at="%s", ' \
               'description="%s"])' % (self.id, self.recipient_id, self.checked_by_id, self.checked_at, self.is_signed,
